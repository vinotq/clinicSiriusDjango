...
class PatientViewSet(viewsets.ModelViewSet):
    ...
    def parents(self, request, pk=None):
        """Получение списка родителей пациента."""
    ...
    def children(self, request, pk=None):
        """Получение списка детей пациента."""
        patient = self.get_object()
        serializer = PatientSerializer(
            patient.children(),
            many=True,
    ...
    def add_parent(self, request, pk=None):
        ...
        if parent.pk == patient.pk:
            return Response(
                {'detail': 'Parent and child must be different'},
                status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def remove_parent(self, request, pk=None):
        ...
        deleted, _ = PatientGroup.objects.filter(
            parent_id=parent_id,
            child=patient
        ).delete()
        if deleted:
            return Response(
                {'detail': 'parent removed'},
    ...
    def me(self, request):
        """Получение профиля текущего пользователя-пациента."""
        parent_patient = getattr(request.user, 'patient_profile', None)
        if not parent_patient:
            return Response(
                {'detail': 'No patient profile for current user'},
                status=status.HTTP_404_NOT_FOUND
            )
        serializer = PatientSerializer(
            parent_patient,
        ...
        data['parents'] = PatientSerializer(
            parent_patient.parents(),
            many=True,
            context={'request': request}
        ).data
        data['children'] = PatientSerializer(
            parent_patient.children(),
            many=True,
            context={'request': request}
        ).data
    ...
    def create_family_member(self, request):
        ...
        if not parent_patient:
            return Response(
                {'detail': 'Parent has no patient profile'},
                status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            context['invites'] = list(
                FamilyInvite.objects.filter(patient=patient, used=False)
            )
        except Exception:
            context['invites'] = []
...
class FamilyInviteView(PatientRequiredMixin, View):
    """Представление для создания приглашения в семью."""
    ...
    def post(self, request):
        ...
        try:
            ...
            return redirect(
                f"{reverse('patients:patient_manage_html')}?invite={code}"
            )
...
class FamilyRedeemView(PatientRequiredMixin, View):
    """Представление для использования кода приглашения."""
    ...
    def post(self, request):
        ...
        try:
            ...
            PatientGroup.objects.get_or_create(
                parent=invite.patient,
                child=patient
            )
            invite.used = True
            invite.save()
...
class RemoveFamilyMemberView(PatientRequiredMixin, View):
    """Представление для удаления члена семьи."""
