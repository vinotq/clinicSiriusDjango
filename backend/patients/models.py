...
class Patient(models.Model):
    ...
    def ancestors(self, max_depth=10):
        """Возвращает всех предков пациента до указанной глубины."""
        results = set()
        current = {self}
        depth = 0
    ...
    def descendants(self, max_depth=10):
        """Возвращает всех потомков пациента до указанной глубины."""
        results = set()
        current = {self}
        depth = 0
        while current and depth < max_depth:
...
class PatientGroup(models.Model):
    """Модель связи родитель-ребенок между пациентами."""

    parent = models.ForeignKey(
        Patient,
        on_delete=models.CASCADE,
        related_name='parents_rel'
    )
    ...
    def clean(self):
        """Валидация: родитель и ребенок должны быть разными пациентами."""
